    ...
    if (placeholder) {
      items.push(
        <button
          ...
          className={`custom-dropdown-option ${value === placeholder.value ? 'selected' : ''} ${placeholder.disabled ? 'disabled' : ''}`}
          onClick={() => handleSelect(placeholder)}
          disabled={placeholder.disabled}
          aria-disabled={placeholder.disabled}
          type="button"
        >
          <span className="custom-dropdown-option-copy">
            <span className="custom-dropdown-option-label">{placeholder.label}</span>
            {placeholder.hint && <span className="custom-dropdown-option-hint">{placeholder.hint}</span>}
          </span>
        ...
        items.push(
          <div key={group.label} className="custom-dropdown-group">
            ...
            {group.options.map((opt) => (
              <button
                key={opt.value}
                className={`custom-dropdown-option grouped ${value === opt.value ? 'selected' : ''} ${opt.disabled ? 'disabled' : ''}`}
              ...
              >
                <span className="custom-dropdown-option-copy">
                  ...
                  {opt.hint && <span className="custom-dropdown-option-hint">{opt.hint}</span>}
                </span>
    ...
    if (options && !optionGroups) {
      options.forEach((opt) => {
        ...
        items.push(
          <button
            key={opt.value}
            className={`custom-dropdown-option ${value === opt.value ? 'selected' : ''} ${opt.disabled ? 'disabled' : ''}`}
            onClick={() => handleSelect(opt)}
            disabled={opt.disabled}
            aria-disabled={opt.disabled}
            type="button"
