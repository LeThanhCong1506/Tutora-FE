...
interface FilterBarProps {
  ...
  // Danh sách cấp học lấy từ API (GET /api/grade-levels).
  gradeLevelOptions: GradeLevelItem[];
  // Các cấp học đang chọn — lưu theo TÊN ("Lớp 10"), khớp đúng gradeName của BE.
  gradeLevels: string[];
  budgetRange: string;
  teachingMode: string;
  city: string;
...
const FilterBar = ({
  ...
  const getGradeLabel = () => {
    ...
    // Giá trị filter chính là gradeName ("Lớp 10") → dùng làm nhãn luôn.
          ...
          <div id="grade-filter-panel" className="filter-options-panel grade-options-panel">
            ...
            {gradeLevelOptions.map((grade) => (
              <button
                ...
                type="button"
                className={`filter-option-chip ${gradeLevels.includes(grade.gradeName) ? 'active' : ''}`}
                onClick={() => handleGradeToggle(grade.gradeName)}
                aria-pressed={gradeLevels.includes(grade.gradeName)}
              >
                {grade.gradeName}
              </button>
            ))}
